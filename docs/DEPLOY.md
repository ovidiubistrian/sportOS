...
REVALIDATE_SECRET=…
# Encrypts the gateway credentials clubs paste in. The API refuses to start in
# production without it, and rotating it makes every stored credential
# unreadable — so keep it with the database password, not separately.
